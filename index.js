...
if (anu.action == 'add') {
uno.sendMessage(anu.id, { document: thumb, fileName: 'Uno MD', fileLength: 0, pageCount: '2023', mentions: [num], caption: `@${num.split("@")[0]} Welcome To ${metadata.subject}` })
} else if (anu.action == 'remove') {
uno.sendMessage(anu.id, { document: thumb, fileName: 'Uno MD', fileLength: 0, pageCount: '2023', mentions: [num], caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
} else if (anu.action == 'promote') {
uno.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
} else if (anu.action == 'demote') {
uno.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
}
}
} catch (err) {
console.log(err)
